...
pub fn load_audio_data(wav_path: &Path) -> Result<Vec<f32>, String> {
...
impl Transcriber {
    ...
    pub fn transcribe_chunk(&self, audio_data: &[f32], offset_seconds: f32) -> Result<Vec<ASRTranscriptChunk>, String> {
        ...
        for i in 0..num_segments {
            ...
            let start_sec = (t0 as f32 / 100.0) + offset_seconds;
            let end_sec = (t1 as f32 / 100.0) + offset_seconds;
            ...
            for j in 0..num_tokens {
                if let Ok(token_data) = state.full_get_token_data(i, j) {
                    if let Ok(token_text) = state.full_get_token_text(i, j) {
                        let token_t0 = (token_data.t0 as f32 / 100.0) + offset_seconds;
                        let token_t1 = (token_data.t1 as f32 / 100.0) + offset_seconds;
