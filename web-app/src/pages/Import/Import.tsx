...
import { Upload, Book, Music, FileJson, ArrowLeft } from 'lucide-react';
import { saveBook, type SyncPoint, type ContentBlock } from '../../storage/db';

export default function Import() {
  const navigate = useNavigate();
  const [epubFile, setEpubFile] = useState<File | null>(null);
  const [audioFile, setAudioFile] = useState<File | null>(null);
  const [syncMapFile, setSyncMapFile] = useState<File | null>(null);
  
  const [isProcessing, setIsProcessing] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleImport = async () => {
    if (!epubFile || !audioFile || !syncMapFile) {
      setError("Please provide all three files to import a book.");
      return;
    }
    ...
    try {
      ...
      // 2. Parse Sync Map
      const syncMapText = await syncMapFile.text();
      let syncMap: SyncPoint[];
      try {
        syncMap = JSON.parse(syncMapText);
      } catch (e) {
        throw new Error("Invalid Sync Map JSON file.", { cause: e });
      }
  ...
  return (
    <div className="container" style={{ paddingTop: '2rem', paddingBottom: '4rem', maxWidth: '800px' }}>
      ...
      <header style={{ marginBottom: '3rem' }}>
        <h1>Import Book</h1>
        <p style={{ color: 'var(--text-secondary)' }}>Upload your EPUB, Audio, and Sync Map to add a new book to your library.</p>
      </header>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', marginBottom: '3rem' }}>
        ...
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '1.5rem' }}>
          ...
          <div style={{ flex: 1 }}>
            <h3 style={{ marginBottom: '0.25rem' }}>Sync Map</h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', marginBottom: '0.5rem' }}>The JSON alignment file</p>
            <input 
              type="file" 
              accept=".json" 
              onChange={(e) => setSyncMapFile(e.target.files?.[0] || null)}
              style={{ width: '100%' }}
            />
          </div>
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <button 
          ...
          disabled={isProcessing || !epubFile || !audioFile || !syncMapFile}
          style={{ padding: '1rem 2rem', fontSize: '1.125rem', opacity: (isProcessing || !epubFile || !audioFile || !syncMapFile) ? 0.5 : 1 }}
        >
